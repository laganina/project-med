...
for fold, (train, test) in enumerate(cv.split(X, y)):
    ...
    #     ax=ax,
    # )
    predictions = classifier.predict_proba(X[test])
    fpr, tpr, _ = metrics.roc_curve(y[test], predictions[:,1])
    roc_auc = metrics.roc_auc_score(y[test], predictions[:,1])
...
plt.savefig('ROC curve Dec Tree.png')   
plt.show()
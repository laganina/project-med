...
classifier = svm.SVC(kernel="linear", C = 21, probability=True, random_state=random_state)
...
for fold, (train, test) in enumerate(cv.split(X, y)):
    print(f'Партиција: {fold}')
    classifier.fit(X[train], y[train])
    viz = RocCurveDisplay.from_estimator(
        classifier,
        X[test],
        y[test],
        name=f"РКП у парт. {fold}",
        alpha=0.3,
        lw=1,
        ax=ax,
    )
...
ax.set(
    ...
    title=f"Средња РКП (ROC-крива) МВН класификатора са варијабилностима\n(ознака позитива: '{1}')",
)
ax.axis("square")
